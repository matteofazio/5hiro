...
class Agent:
	...
	def buy(self, short=False):
		...
		return [True,f"{strategy} (Close:{self.Strategy.data.Close.iloc[-1]}), sl:{self.Strategy.data.Close.iloc[-1]*0.98}, tp:{self.Strategy.data.Close.iloc[-1]*(1+0.02*1.5)}"] # da cambiare in self.strategy
